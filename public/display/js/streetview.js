...
define(
	['config', 'bigl', 'validate', 'stapes', 'googlemaps', 'sv_svc', 'tour'],
	function(config, L, validate, Stapes, GMaps, sv_svc, tourProvider) {
		var StreetViewModule = Stapes.subclass({
			...
			constructor: function($canvas, master, zoom, tourId) {
				this.$canvas = $canvas;
				this.master = master;
				this.map = null;
			...
			svCustomPanoProvider: function(pano) {
				return this.svPanoramas[pano];
			},
			// PUBLIC
			...
			init: function() {
				...
				if(this.tour) {
					svOptions.panoProvider = this.svCustomPanoProvider;
				}
				...
				if (this.master) {
					...
					if (this.tour) {
						this.pano = this.tour.panoramas[0].filename;
			...
			setTour: function(tourId) {
				if (!this.tour || tourId !== this.tour.id) {
					this.tour = this.loadTour(tourId);
					this.tour.id = tourId;
					this.svPanoramas= this.tour.panoramas.map(function(panorama) {
						return {
							tourPano: panorama,
							location: {
							...
							tiles: {
								...
								// tile set.
								centerHeading: 105,
								getTileUrl: panorama.fileUrls.FULL
							}
						};
					});					console.info('StreetView: Loaded new tour ' + tourId);
				} else {
					console.warn('StreetView: ignoring redundant setTour');
				}
			},
