...
requirejs.config({
  paths: {
    ...
    'immoviewertour': '/js/immoviewertour',
  ...
  shim: {
    ...
    'googlemaps': {
      deps: [
        'async!http://maps.googleapis.com/maps/api/js?v=3.26!callback'
      ]
    }
  },
  map: {
...
require(
	['fields', 'immoviewertour', 'streetview', 'customtour', 'viewsync', 'multiaxis'],
	function(
		fields,
		immoviewer,
		StreetViewModule,
...
// *** initialize the ViewSync module
  ...
  // *** initialize the StreetView module
	var sv;
	if(LOCAL_CONFIG.tour) {
		immoviewer(LOCAL_CONFIG.tour, function(panos) {
			sv = new CustomTourModule(
				document.getElementById('pano'),
				LOCAL_CONFIG.master,
				LOCAL_CONFIG.zoom,
		...
		sv = new StreetViewModule(
			document.getElementById('pano'),
			LOCAL_CONFIG.master,
			LOCAL_CONFIG.zoom
		);
		start(LOCAL_CONFIG.pano);
	}
